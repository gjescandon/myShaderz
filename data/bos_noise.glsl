...
void main() {
	...
	if( p.x<0.5 && p.y < 0.5)
	{
		f = noiseValue( 32.0*uv );
	}
    // top: fbm - fractal noise (4 octaves)
    if( p.x<0.5 && p.y > 0.5)	    	
	{
		uv *= 8.0;
        mat2 m = mat2( 1.6,  1.2, -1.2,  1.6 );
		f  = 0.5000*noiseValue( uv ); uv = m*uv;
		f += 0.2500*noiseValue( uv ); uv = m*uv;
		f += 0.1250*noiseValue( uv ); uv = m*uv;
		f += 0.0625*noiseValue( uv ); uv = m*uv;
	}

    // right: gradient noise

    // bottom: noise	
	if( p.x>0.5 && p.y < 0.5)
	{
		f = noiseGradient( 32.0*uv );
	}
    // top: fractal noise (4 octaves)
	if( p.x>0.5 && p.y > 0.5)
	{
		uv *= 8.0;
        mat2 m = mat2( 1.6,  1.2, -1.2,  1.6 );
		f  = 0.5000*noiseGradient( uv ); uv = m*uv;
		f += 0.2500*noiseGradient( uv ); uv = m*uv;
		f += 0.1250*noiseGradient( uv ); uv = m*uv;
		f += 0.0625*noiseGradient( uv ); uv = m*uv;
	}
    

	f = 0.5 + 0.5*f;
	
    f *= smoothstep( 0.0, 0.005, abs(p.y-0.5) );	
