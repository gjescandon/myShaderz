...
void main( void )
{
    ...
    vec3 col = vec3(fract(p.x));
    float nf = noize3(abs(1.+p.x) + 0.6*iTime);
    if (nf > 0.1) {
      if ((fract(p.y) > 0.25)) col = vec3(getColorStep(fract(iRandom1 + p.x+ 0.3*iTime)));
      if ( (p.y > 0.5)) col = vec3(getColorSlow(fract(iRandom2 + p.x+ 0.3*iTime)));
      if ((p.y > 0.75)) col = vec3(getColorFast(fract(iRandom3 + p.x+ 0.3*iTime)));
    }
